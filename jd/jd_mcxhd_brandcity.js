            ...
            for (j = 0; j < $.taskList.length; j++) {
                ...
                if ($.taskList[j].status === 2) {
                    ...
                    for (k = 0; k < $.taskList[j].subItem.length; k++) {
                        ...
                        let taskToken = $.taskList[j].subItem[k].itemToken
                        await doTask(taskToken, taskType)
                        await $.wait(1000);
                        //                 console.log(taskType)
                        if (taskType === "9") {
                            taskToken = $.taskList[j].subItem[k].taskToken
                            await doTask(taskToken, "8")
                            await $.wait(5000);
                            await doTask($.taskList[j].subItem[k].itemToken, taskType, 6)
                        }
                    }
                }
...
function doTask(imToken, taskType, check) {
    return new Promise(async (resolve) => {
        let options = taskPostUrl(`mcxhd_brandcity_doTask`, `{"itemToken":"${imToken}","token":"jd17919499fb7031e5"}`)
        if (taskType === "8") {
            options = taskPostUrl("qryViewkitCallbackResult", `{"dataSource":"newshortAward","method":"getTaskAward","reqParams":"{\"taskToken\":\"${imToken}\"}","sdkVersion":"1.0.0","clientLanguage":"zh"}`)
        }
        if (check) {
            options = taskPostUrl("mcxhd_brandcity_checkTaskStatus", `{"itemToken":"${imToken}","token":"jd17919499fb7031e5"}`)
        }
        //   console.log(options)
        $.post(options, async (err, resp, data) => {
            try {
...
function taskPostUrl(functionid, body) {
    return {
        url: `https://api.m.jd.com/?body=${body}&appid=publicUseApi&functionId=${functionid}&t=${Date.now()}`,
        body: "",
        headers: {
