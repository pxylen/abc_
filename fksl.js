...
!(async () => {
  ...
  } else {fkslbodyArr.push($.getdata('fkslbody'))
    fkslhdArr.push($.getdata('fkslhd'))
    fkslurlArr.push($.getdata('fkslurl'))
    let fkslcount = ($.getval('fkslcount') || '1');
  for (let i = 2; i <= fkslcount; i++) {
    fkslbodyArr.push($.getdata(`fkslbody${i}`))
    fkslurlArr.push($.getdata(`fkslurl${i}`))
    fkslhdArr.push($.getdata(`fkslhd${i}`))
  }
