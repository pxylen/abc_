/*软件名：金钱豹  扫码下载  
注意，必须扫码使用我发的二维码扫码打开软件，否则不会加载出任务界面
本脚本以学习为主，出现任何情况与本人无关。
脚本包含除了试玩任务外的所有任务
任务完成每天收益0.5元左右
两元可提现
...
[task_local]
#金钱豹
15 10 * * * https://raw.githubusercontent.com/age174/-/main/jqb.js, tag=金钱豹, img-url=https://s3.ax1x.com/2021/02/06/yYzYWR.png, enabled=true
...
//金钱豹数据获取
function jqbck() {
   if ($request.url.indexOf("getintegrals") > -1&&$request.body.indexOf("adconfigid=2") > -1){
  $.setdata(JSON.stringify($request.url),'jqburl')
    $.log(jqburl)
    $.setdata(JSON.stringify($request.headers),'jqbhd')
	   $.msg($.name,"","金钱豹headers获取成功！")
$.log(jqbhd)
...
$.log(jqbzpbody)
  ...
  }else if ($request.url.indexOf("submitsign") > -1){
...
let url = {
    ...
    const result = JSON.parse(data)
        if(result.statuscode == 200){
        console.log('金钱豹签到回执:成功🌝 '+result.msg)
}
if(result.statuscode == 400 || result.statuscode == 401){
