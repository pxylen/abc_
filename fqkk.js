...
别几天就把羊薅死了，账号多的大佬觉得运行一次太久的话也可以去boxjs自行修改循环次数，比如修改循环次数为20，则每天运行五次脚本，循环次数为50则每天要运行两次脚本。。默认为循环次数为100一天运行一次，反正不管怎么修改，尽量每天循环次数别超过100，目前最高一天阅读120次
...
!(async () => {
  ...
  for (let i = 2; i <= fqkkcount; i++) {
    ...
    console.log(`\n番茄看看当前设置循环次数为: `+fqkkxh+'次，提现金额为: '+fqtx / 100+' 元')
